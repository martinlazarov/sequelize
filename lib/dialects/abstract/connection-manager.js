...
/*
  ...
  */
    this.pool = new ConnectionPool(this.config, this.dialect, this.sequelize);
  }
  ...
  getConnection(options) {
    ...
    if (this.sequelize.options.databaseVersion === 0) {
      if (this.versionPromise) {
        ...
        promise = this.versionPromise = this._connect(this.config.databases && options.dbName ? this.config.databases[options.dbName] : this.config)
        .then(connection => {
            const _options = {};
    ...
    return promise.then(() => {
      return this.pool.acquire(options.priority, options.type, options.dbName ? options.dbName : "master")
        .then(mayBeConnection => this._determineConnection(mayBeConnection))
